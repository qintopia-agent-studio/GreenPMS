-- Operator-facing collection methods now drive transaction-reference rules:
-- WECOM and BANK_TRANSFER require a reference; CASH and OTHER may record
-- without one (their required business note is enforced at the command layer).
...
BEGIN
  ...
  IF NEW.fact_type IN ('COLLECTION', 'REFUND')
    AND NEW.method IN ('WECOM', 'BANK_TRANSFER')
    AND NEW.transaction_reference IS NULL THEN
    RAISE EXCEPTION 'wecom and bank transfer facts require a transaction reference'
