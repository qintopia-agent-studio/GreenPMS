-- Stay-to-membership conversion relaxations:
--   1. In-house stays may be converted (previously only completed/check-out
--      stays), so the order state guard now accepts CHECKED_IN + IN_HOUSE.
--   2. Orders with zero recorded collections may be converted without any
--      transfer rows; the membership fee is then collected in full as a
--      direct WeCom payment. The funds-conservation equations already
--      enforce net lodging zero and full direct coverage in that case.
...
BEGIN
  ...
  IF execution_property_id IS DISTINCT FROM target_order.property_id
    ...
    RAISE EXCEPTION 'stay-to-membership conversion must leave one in-house or completed ordinary lodging order at zero amount'
