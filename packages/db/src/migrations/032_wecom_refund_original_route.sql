-- WECOM refunds are original-route refunds: the external transaction number is
-- the referenced WECOM collection's transaction number, so operators must not
-- be forced to type a new refund transaction reference.
