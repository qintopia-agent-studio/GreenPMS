...
trap 'docker exec "$container" dropdb -U "$migration_user" --if-exists "$source_database" >/dev/null 2>&1 || true; docker exec "$container" dropdb -U "$migration_user" --if-exists "$target_database" >/dev/null 2>&1 || true; rm -rf "$workdir"' EXIT
...
run_app_command() {
  ...
  DATABASE_URL="" PGHOST="127.0.0.1" PGPORT="$host_port" PGDATABASE="$database" PGUSER="$migration_user" PGPASSWORD="$migration_password" "$@"
}

docker exec "$container" createdb -U "$migration_user" "$source_database"
docker exec "$container" psql -U "$migration_user" -d "$source_database" -v ON_ERROR_STOP=1 \
...
for migration_path in "$migrations_directory"/[0-9][0-9][0-9]_*.sql; do
  ...
  } | docker exec -i "$container" psql -U "$migration_user" -d "$source_database" -v ON_ERROR_STOP=1 >/dev/null
  source_migration_count=$((source_migration_count + 1))
done
test "$source_migration_count" = "27"
source_stage10_migration_count="$(docker exec "$container" psql -U "$migration_user" -d "$source_database" -Atc "SELECT count(*) FROM schema_migrations WHERE name ~ '^[0-9]{3}_.*[.]sql$'")"
test "$source_stage10_migration_count" = "27"
test "$(docker exec "$container" psql -U "$migration_user" -d "$source_database" -Atc "SELECT count(*) FROM schema_migrations WHERE name = '027_stage10_stay_shortening_guards.sql'")" = "1"
test "$(docker exec "$container" psql -U "$migration_user" -d "$source_database" -Atc "SELECT count(*) FROM schema_migrations WHERE name = '028_stage11_move_unit_guards.sql'")" = "0"
docker exec "$container" psql -U "$migration_user" -d "$source_database" -v ON_ERROR_STOP=1 -c "INSERT INTO properties(id, code, name, timezone, currency) VALUES ('$sentinel_property_id', '$sentinel_property_id', 'Restore sentinel', 'Asia/Shanghai', 'CNY')" >/dev/null
...
member_property_link_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc 'SELECT count(*) FROM member_property_links')"
operational_reference_columns="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM information_schema.columns WHERE (table_name = 'orders' AND column_name IN ('booking_channel_code','channel_order_reference')) OR (table_name = 'collection_facts' AND column_name = 'transaction_reference')")"
operational_reference_triggers="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM pg_trigger WHERE NOT tgisinternal AND tgname IN ('orders_validate_new_channel','collection_facts_validate_new_transaction_reference')")"
one_stay_violations="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc 'SELECT count(*) FROM orders o LEFT JOIN stays s ON s.order_id=o.id GROUP BY o.id HAVING count(s.id) <> 1' | wc -l | tr -d ' ')"
missing_revisions="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc 'SELECT count(*) FROM orders WHERE current_revision_id IS NULL')"
room_bed_conflicts="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc 'SELECT count(*) FROM inventory_room_days r JOIN inventory_bed_days b ON b.room_id=r.room_id AND b.service_date=r.service_date WHERE r.whole_claim_id IS NOT NULL AND b.bed_claim_id IS NOT NULL')"
catalog_batch_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM catalog_import_batches WHERE id = '$catalog_batch_id' AND sealed_at IS NOT NULL AND jsonb_typeof(source_snapshot) = 'object' AND length(content_hash) = 64")"
catalog_inventory_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM inventory_catalog_entries WHERE import_batch_id = '$catalog_batch_id'")"
catalog_rate_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM reference_rate_entries WHERE import_batch_id = '$catalog_batch_id'")"
catalog_membership_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM reference_membership_products WHERE import_batch_id = '$catalog_batch_id'")"
target_catalog_content_hash="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT content_hash FROM catalog_import_batches WHERE id = '$catalog_batch_id'")"
target_catalog_snapshot_hash="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT md5(source_snapshot::text) FROM catalog_import_batches WHERE id = '$catalog_batch_id'")"
fixture_order_id="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT orders.id FROM orders JOIN order_occupants ON order_occupants.order_id = orders.id AND order_occupants.role = 'PRIMARY' WHERE orders.booking_channel_code = 'WECOM' AND orders.channel_order_reference IS NULL AND order_occupants.document_number = '$fixture_reference'")"
fixture_stay_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM stays WHERE order_id = '$fixture_order_id'")"
fixture_amendment_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM amendments WHERE order_id = '$fixture_order_id'")"
fixture_revision_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM pricing_revisions WHERE order_id = '$fixture_order_id'")"
fixture_claim_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM inventory_claims WHERE source_type = 'ORDER_SEGMENT' AND source_id IN (SELECT id FROM stay_segments WHERE stay_id IN (SELECT id FROM stays WHERE order_id = '$fixture_order_id')) AND active")"
fixture_fact_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM collection_facts WHERE order_id = '$fixture_order_id' AND transaction_reference IN ('$fixture_reference-COLLECTION-1', '$fixture_reference-COLLECTION-2', '$fixture_reference-REFUND-1')")"
fixture_refund_reference_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM collection_facts refund JOIN collection_facts collection ON collection.fact_id = refund.references_fact_id WHERE refund.order_id = '$fixture_order_id' AND refund.fact_type = 'REFUND' AND refund.transaction_reference = '$fixture_reference-REFUND-1' AND collection.order_id = refund.order_id AND collection.fact_type = 'COLLECTION' AND collection.transaction_reference = '$fixture_reference-COLLECTION-1'")"
fixture_receipt_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM command_receipts receipt JOIN command_executions execution ON execution.id = receipt.command_id WHERE execution.command_type IN ('CREATE_ORDER','RECORD_COLLECTION','RECORD_REFUND') AND receipt.business_committed AND receipt.result ->> 'orderId' = '$fixture_order_id'")"
fixture_audit_count="$(docker exec "$container" psql -U "$migration_user" -d "$target_database" -Atc "SELECT count(*) FROM audit_entries WHERE command_id IN (SELECT command_id FROM collection_facts WHERE order_id = '$fixture_order_id') AND decision = 'ALLOWED'")"

test "$source_marker" = "1"
test "$target_marker" = "0"
test "$target_sentinel" = "1"
test "$required_migrations" = "$current_migration_count"
