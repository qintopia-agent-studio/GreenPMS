...
run_current_readiness() {
  ...
  DATABASE_URL="" PGHOST="127.0.0.1" PGPORT="$host_port" PGDATABASE="$target" PGUSER="$user" PGPASSWORD="$password" npm run db:ready
}
...
restored_migration_manifest="$(docker exec "$container" psql -U "$user" -d "$target" -At -v ON_ERROR_STOP=1 -c "SELECT count(*)::text || ':' || COALESCE(string_agg(name, ',' ORDER BY name), '') FROM schema_migrations")"
