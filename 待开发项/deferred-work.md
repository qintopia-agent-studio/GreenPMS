- source_spec: `待开发项/QinTopia-PMS-第4步-4.3-实施规格.md`
  summary: 渠道价格差异超过 15% 且缺少说明时显示具体补填原因、字段错误与定位按钮（2026-09-21 已实现，待反馈）。
  evidence: TypeScript、147 项相关单元及价格边界测试、真实 CTRIP 缩短住宿浏览器回归与构建通过；原计价与提交规则保持既定规格。
...
- source_spec: `docs/releases/v1.1.0.md`
  summary: 2026-09-07 14:59 v1.1.0 已部署，应用提交 355ee90，包含整房同住人管理、Logo 下方版本号和 OpenAPI 声明修复。
  evidence: 用户已明确批准向现有公开仓库发布并部署。隔离恢复演练、迁移 056、30 张业务及会话表摘要一致、受限身份 readiness、公网健康与版本验证通过；桌面员工及手机管理员的同住人表单、7 天登录、刷新、退出及版本布局通过。生产未录入测试人员；服务器与本地备份一致。下文未发布状态为历史记录，人工逐项反馈仍待反馈。
...
- source_spec: 2026-09-12 用户提供的 iPad 投屏房态截图
  summary: 修复日期栏与筛选栏之间的响应式吸顶空白。
  implementation_status: 已实现，待反馈；尚未发布。日期栏偏移改为 ResizeObserver 读取筛选栏实际高度，移除 83px / 154px 固定高度，适配窗口变化及更多筛选展开/收起。
  evidence: 2026-09-15 接入最新主线后，TypeScript、PR 格式校验器 8 项及 git diff --check 通过；真实 Toolbar 与项目 CSS 的 Chromium 隔离布局复查覆盖 768、820、1024、1194、1280、1366、1440、1920px，筛选收起、展开、生效、再次收起共 32 场景，滚动吸顶间隙均为 0px，并确认主线新增的自定义房型名称保留。未执行业务 E2E，iPad Safari 实机投屏待反馈。合入代码不代表发布或人工验收通过。
- source_spec: 用户2026-09-11反馈：侧栏折叠时恢复Q，展开时保留系统名称和版本号。
  summary: 已实现，待用户线上反馈；仅修正折叠品牌与版本号CSS，保留窄屏顶部完整品牌。
  evidence: 本地生产构建通过；真实浏览器使用合成会话检查1440px展开/折叠、刷新保留折叠、再次展开及375px顶部，全部通过且无页面异常。未新增业务测试或修改接口/数据。

- source_spec: `待开发项/QinTopia-PMS-房型房间床位与价格管理-实施规格.md`
  summary: 2026-09-15 房型、房间床位与统一价格管理已实现，自动检查和浏览器检查通过，用户人工验收待执行，未部署。
  evidence: 独立分支 `codex/room-types-pricing-settings`；设置入口 `/settings/rooms`，删除/停用、入住日选价、旧订单锁价、管理员会话权限按已确认 D1-D3 实施。主验收计划末尾提供 3 组操作与预期结果，本地合成验收地址 http://127.0.0.1:4243/settings/rooms 。

- source_spec: 用户 2026-09-15 反馈：账号、外部访问与新增设置页的导航必须一致且可找回。
  summary: 已实现，待用户反馈；设置总览和各子页统一固定导航，保留账号、外部访问及房型与价格的固定顺序和总览入口。页面懒加载仅替换内容区，主导航与设置导航保留。
  evidence: TypeScript、Web 构建和既有导航/Token 相关 17 项测试通过；浏览器验证各设置页来回切换、旧 /accounts 与 /tokens 地址、刷新及前进后退；窄屏入口完整且无页面横向溢出，长列表定位末尾后设置导航仍在顶部 56px。原有角色可见性和后端权限保持既有规则。后续新增页面须一并检查导航、原入口和返回路径，不能只验新增页。

- source_spec: 用户 2026-09-15 确认的设置导航方案。
  summary: 已实现，待用户反馈；设置首页恢复三个卡片，子页按相同名称及顺序提供房型与价格、账号管理、智能体与外部访问导航，并保留返回设置。替代此前仅有设置总览标签的布局。
  evidence: TypeScript、构建、既有导航与 Token 相关 17 项测试通过；浏览器完成三个卡片往返、子页互跳、旧入口重定向、前进后退和刷新、手机入口及长页固定导航验证。既有 E2E 的标题和导航名称选择器同步更新，未重跑完整 E2E。设置内容的加载和渲染错误隔离在导航下方；完整人工验收用例见 `待开发项/QinTopia-PMS-房型与价格-人工验收用例.md`。

- source_spec: 用户 2026-09-16 确认：订单列表中央应能滚动页面，AI 助手和订单操作抽屉均支持点击外部收起。
  summary: 已实现，待反馈；订单列表纵向滚动向页面传递，保留横向滚动；AI 助手及 drawer 抽屉支持外部点击和 Esc 收起。此决定更新 drawer 此前仅显式关闭的交互约定，普通弹窗仍保留原规则。
  evidence: 本地预览 http://127.0.0.1:4236/orders 。真实 Chromium 覆盖 50 行合成列表上下滚轮、桌面/平板助手外部点击和输入保留、手机全屏助手 Esc、退房日期/换房抽屉内外点击、编辑后取消/确认关闭、内向外拖动不误关；真实 Modal 组件隔离检查 closeDisabled 和非模态抽屉通过。编辑过表单（含按钮选择）的抽屉外部关闭前提示放弃修改，提交中沿用 closeDisabled；不改命令、恢复记录或业务数据。相关当前目录 215 项单元测试、TypeScript 与构建通过；脚本、组件验证夹具和截图保留在已忽略的 .local-workspace/order-interactions/。未发布，人工反馈不等于自动检查通过。

- source_spec: 用户 2026-09-17 反馈：点击 AI 助手五个默认问题中的任一个，抽屉立即误收起。
  summary: 已实现，待反馈；外部点击判断改用原始事件路径，避免默认问题按钮在发送后卸载，被误认为在抽屉外点击。
  evidence: 新增真实浏览器回归先在旧实现复现失败，修复后桌面和手机逐个验证五个默认问题，覆盖点击标题/说明和键盘激活；等待答复与展示答复时抽屉保持打开，外部点击、Esc、输入法及发送交互回归共 6 项通过。模型请求使用模拟答复，不调用真实模型。TypeScript 与 Web 构建通过；本地预览 http://127.0.0.1:4236/orders ，未发布到生产。

- source_spec: 用户 2026-09-17 确认：处理同一个问题时，跳转及填写业务操作页不能自动收起 AI 助手。
  summary: 已实现，待反馈；此决定替代此前 AI 助手点击外部收起的规则。助手仅通过关闭按钮、再次点击入口或 Esc 收起；跳转、打开/关闭业务表单和点击业务区域均保留助手、对话与输入。桌面为助手预留空间并与操作表单并排，窄屏上下展示。
  evidence: 助手挂载到当前原生模态层内，保持业务模态边界与表单关闭保护；跨页跳转不再提前清除待打开的操作入口。桌面/手机 10 项浏览器检查、TypeScript、构建与 diff 检查通过，覆盖五个默认问题、跨页草稿保留、表单内继续追问使用同一 conversationId、两区域可操作且不重叠、关闭助手后业务草稿保留、关闭表单后助手保留；测试模拟模型答复，未调用真实模型。旧建单测试夹具固定价格版本已过期，本轮通过 ASSISTANT_TEST_ORDER_ID 复用既有合成订单完成真实表单验证，未提交业务操作。截图在 .local-workspace/order-interactions/persistent-final/，预览 http://127.0.0.1:4236/orders ，未发布。
- source_spec: 用户 2026-09-21 要求将业务失败提示专项检查发现的五项问题一批修完。
  summary: 已实现，待反馈；改期/换房显示本地校验原因，相同政策金额提供恢复入口，超 366 夜明确说明上限，换房刷新展示实际失败原因并标识旧状态失效，只读加载使用独立错误文案。
  evidence: TypeScript、1269 项单元、生产构建及 8 项 PR 格式测试通过；独立合成库 5 项浏览器回归覆盖真实计价拒绝、补填/切回系统金额后的恢复、换房刷新错误、只读 404/断网及既有渠道/追溯日期门禁。换房刷新错误使用注入响应，初次核对与恢复核对使用真实 API；计价、库存与提交规则未改动。验收步骤见主验收计划“2026-09-21 业务失败提示五项修复”，未合并、未部署。
