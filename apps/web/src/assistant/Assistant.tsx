...
import { MessageSquare, Plus, Send, Settings, Sparkles, X } from "lucide-react";
import { api } from "../api";
...
export function AssistantProvider({ children }: { children: ReactNode }) {
  const { propertyId, meta } = useWorkspace(), location = useLocation(), navigate = useNavigate();
  ...
  const propertyName = meta.properties.find(p => p.id === propertyId)?.name ?? "当前门店";
  ...
  const newConversation = () => { generation.current++; controller.current?.abort(); setBusy(false); setConversationId(undefined); setMessages([]); setError(undefined); setGuide(undefined); setPending(undefined); refreshSettings(); };
  ...
  return <AssistantContext.Provider value={{ open, toggle, settings, refreshSettings, guide, pending, finishEntry }}>
    ...
    {open && !dialogOpen ? <aside id="ai-assistant-panel" className="assistant-panel" role={mobile ? "dialog" : "complementary"} aria-modal={mobile ? true : undefined} aria-label="AI 助手" data-testid="ai-assistant-panel">
      ...
      <div className="assistant-context"><span>{propertyName}</span><span>{pageName}{orderId ? " · 当前订单" : ""}</span></div>
      <div className="assistant-messages" ref={messagesRef} aria-live="polite" aria-busy={busy}>
        {!messages.length ? <div className="assistant-welcome"><MessageSquare size={27} aria-hidden="true" /><h2>需要帮你做什么？</h2><p>问我怎么操作，或让我查找房态、订单与会员资料。</p>{settings && !settings.enabled ? <p className="assistant-notice">助手尚未启用，请管理员在设置中配置模型连接。</p> : null}<div className="assistant-suggestions">{[orderId ? "打开这个订单的续住入口，并告诉我怎么操作" : "如何给客人办理续住？", "帮我打开会员查询入口"].map(prompt => <button type="button" key={prompt} onClick={() => void send(undefined, prompt)} disabled={busy || !settings?.enabled}>{prompt}</button>)}</div></div> : messages.map((m, i) => <article className={`assistant-message assistant-message-${m.role}`} key={i}><span className="assistant-message-author">{m.role === "user" ? "你" : "AI 助手"}</span><div className="assistant-message-text">{m.text}</div>{m.entries?.map((entry, index) => <div className="assistant-entry" key={index}><button type="button" className="button button-secondary" onClick={() => openEntry(entry)}>打开{entry.label}</button><ol>{entry.steps.map(step => <li key={step}>{step}</li>)}</ol></div>)}</article>)}
        {busy ? <p className="assistant-wait">正在查询和整理…</p> : null}
        {error ? <div className="assistant-error" role="alert">{error}</div> : null}
      </div>
      <form className="assistant-composer" onSubmit={event => void send(event)}><label className="sr-only" htmlFor="assistant-question">向 AI 助手提问</label><textarea id="assistant-question" ref={inputRef} value={draft} onChange={e => setDraft(e.target.value)} maxLength={4000} placeholder="描述你想完成的事情…" rows={3} /><div><small>查询和引导 · 业务操作由你确认</small><button className="button button-primary" type="submit" disabled={busy || !draft.trim() || !settings?.enabled}><Send size={16} aria-hidden="true" />发送</button></div></form>
    </aside> : null}
