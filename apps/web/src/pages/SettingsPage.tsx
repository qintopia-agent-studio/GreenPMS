...
export function SettingsPage() {
  ...
  return <div className="settings-page">
    ...
    <nav className={index ? "settings-cards" : "settings-tabs"} aria-label="设置导航">
      {assistant?.settings?.canManage ? <NavLink to="/settings/ai" className="settings-link"><Sparkles aria-hidden="true" size={20} /><span><strong>AI 助手</strong>{index ? <small>模型服务、连接测试与启停</small> : null}</span></NavLink> : null}
