...
**订单改期与履约边界：** 调整已预订订单的入住日、退房日或两者时保留同一 `orderId/stayId`，不得删除、取消后重建，也不得覆盖原 segment。确认在同一事务追加 amendment、住宿安排版本和完整 pricing revision，迁移 Claim，并释放/重建仍为 HELD 的会员覆盖。已入住后缩短不会恢复 CHECK_IN 时已经 CONSUMED 的权益。阶段 10 对在住缩短严格区分 `newDepartureDate > businessDate`（继续在住）、`=` 且 `arrivalDate < businessDate`（原子提前退房并立即释放营业日及以后库存）和 `< businessDate`（拒绝追溯改写）。入住当天不提供提前退房或钟点房模型；未实际使用房间的“撤销入住”由阶段 12 以独立命令保留原入住/核销历史、追加权益补偿并立即释放库存，实际退款仍由阶段 13 登记。
